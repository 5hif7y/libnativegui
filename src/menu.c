...
#include <math.h>
#include <ctype.h>
#include "neuzeichnen.h" /* for neuzeichnen information */
#include "mygraph.h"
...
static int curve2dScaling = 1,curve3dScaling = 1;
static double curve2dxmin=-1,curve2dxmax=1,curve2dymin=-1,curve2dymax=1,
  curve3dxmin=-1,curve3dxmax=1,curve3dymin=-1,curve3dymax=1,curve3dzmin=-1,curve3dzmax=1;
...
void SetDefault3dScaling(int scaling, double xmin, double ymin, double zmin, 
			 double xmax, double ymax,double zmax){
  curve3dScaling=scaling;
  curve3dxmin=xmin;
  curve3dxmax=xmax;
  curve3dymin=ymin;
  curve3dymax=ymax;
...
{
  extern mywindow* windows;
  extern int nowindows;
  mywindow* mw = NULL;
  int item_idx = -1;
  for (int i = 0; i < nowindows + 1; i++) {
      if (windows[i].installed) {
          for (int j = 0; j < windows[i].nomenu; j++) {
              if (windows[i].menuw[j] == w) {
                  mw = &windows[i];
                  item_idx = j;
                  break;
              }
  ...
  if (!mw || item_idx == -1) return;

  int y1 = item_idx * mw->menumaxy / mw->nomenu;
  int yh = mw->menumaxy / mw->nomenu;

  uint32_t bg_color = 0xFF2A2A2A;
  uint32_t border_color = 0xFF444444;
  uint32_t text_color = 0xFFE0E0E0;

  GW_FillRect(mw->myw, 0, y1, mw->menumaxx, yh, bg_color);
  GW_DrawRect(mw->myw, 0, y1, mw->menumaxx, yh, border_color);

  wchar_t wtext[512] = {0};
  GW_UTF8ToWide(str, wtext, 512);

  int tw = 0, th = 0;
  extern GW_Font* g_font_ui;
  GW_MeasureText(g_font_ui, wtext, &tw, &th);

  int tx = (mw->menumaxx - tw) / 2;
  int ty = y1 + (yh - th) / 2;

  GW_DrawText(mw->myw, g_font_ui, tx, ty, wtext, text_color);
  GW_Present(mw->myw);
}
		      ...
		      XFontStruct *font,char *str1,char *str2,char *str3)
{
  ...
  GW_FillRect(mw->myw, 0, y1, mw->menumaxx, yh, 0xFF000000);
  GW_DrawRect(mw->myw, 0, y1, mw->menumaxx, yh, 0xFF444444);

  char full_text[512];
  snprintf(full_text, sizeof(full_text), "%s%s%s", str1, str2, str3);

  wchar_t wtext[512] = {0};
  GW_UTF8ToWide(full_text, wtext, 512);
  ...
  GW_DrawText(mw->myw, g_font_ui, tx, ty, wtext, 0xFFFFFFFF);
  GW_Present(mw->myw);
}
...
{
    // Resizing is handled via LibGW expose/resize event loop in mygraph.c
}

void mapwindow(Display *myd,mywindow *mw,int myx, int myy)
{
  int i,ii,l,j;
  char longstr[100] = "Colin = 1237.234e3";
  ...
  if(mw->type&menutypemask) 
    {
      mw->menumaxx = l * 8 + 30;
      if (mw->menumaxx < 150) mw->menumaxx = 150;
      mw->menumaxy = mw->nomenu * 35;
  ...
  int w = mw->menumaxx + mw->zeichenmaxx;
  int h = mw->menumaxy;

  mw->myw = GW_CreateWindow(mw->Name, w, h);
  if (!mw->myw) {
      fprintf(stderr, "Failed to create LibGW window: %s\n", mw->Name);
      exit(1);
  }
  mw->zeichenw = mw->myw;
  mw->mypixmap = mw->myw;
  ...
  if (mw->type&menutypemask)
    {
      for (i=0; i<mw->nomenu; i++)
	mw->menuw[i] = (Drawable)(1000 + mw->number * 100 + i);
    }
...
static XFontStruct single_font_struct;
...
void initfonts(Display **myd)
{
  *myd = (Display)1;
  ...
  extern GW_Font* g_font_ui;
  single_font_struct.gw_font = g_font_ui;
  single_font_struct.ascent = 12;
  single_font_struct.descent = 4;
  
  nofoundfonts = 1;
  menfonts[0] = &single_font_struct;
  defaultfont = &single_font_struct;
}
...
void GetNewValue(mywindow *mw,int i,char *data,int numeric)
{
  ...
  while(done==0){
    ...
    if (GW_WaitEvent(&ev)) {
        ...
        else if (ev.type == GW_EVENT_KEY_DOWN) {
            int key = ev.key.keycode;
            if (key == GW_KEY_BACKSPACE) {
                if (text1[0]!='\0') text1[strlen(text1)-1]='\0';
            }
            else if (key == GW_KEY_DELETE) {
                if (text2[0]!='\0'){
                    text2[0]=text3[0];
                    memmove(text3,text3+1,strlen(text3)*sizeof(char));
                }
            }
                ...
                strcpy(data,text1);
                int len = strlen(data);
                if (len > 0 && data[len - 1] == ' ') data[len - 1] = '\0';
                return;
            }
            else {
                if (key >= 32 && key <= 126) {
...
void genericmenu(mywindow *mw,int i,int front,int button,int Shift,int Ctrl)
{
  ...
  case int_:
	...
	*(mw->menu->mydata[i].md.choice.myint.i)+=
	...
	if (front==0) *(mw->menu->mydata[i].md.choice.myint.i)=k/2; 
	  ...
	  if (*(mw->menu->mydata[i].md.choice.myint.i)==k) 
	    *(mw->menu->mydata[i].md.choice.myint.i)=k*2;
	  else *(mw->menu->mydata[i].md.choice.myint.i)=k;
	break;
      }
    else {
  ...
  case long_:
	...
	*(mw->menu->mydata[i].md.choice.mylong.i)+=
	...
	if (front==0) *(mw->menu->mydata[i].md.choice.mylong.i)=k/2; 
	  ...
	  if (*(mw->menu->mydata[i].md.choice.mylong.i)==k) 
  ...
  case boolean_:{
    ...
    (*(mw->menu->mydata[i].md.choice.myint.i))=0;
    else (*(mw->menu->mydata[i].md.choice.myint.i))=1;
    if (*(mw->menu->mydata[i].md.choice.myint.i))
  ...
  case curve2d_:
    ...
    for (k=0;(mw->menu->mydata[i].md.choice.mygraph->anzN_RxRxRxR)>k;k++)
    ...
    sprintf(tmpzd->graph2d.Xlabel,"x");
    sprintf(tmpzd->graph2d.Ylabel,"y");
    ...
    StartMenu("Control",0);
      ...
      DefineString("X label ",tmpzd->graph2d.Xlabel,500);
      DefineString("Y label ",tmpzd->graph2d.Ylabel,500);
  ...
  case curve3d_:
    tmpzd=(nzdata *) malloc(sizeof(nzdata));
    tmpzd->graph3d.comments=0;
    tmpzd->graph3d.newdata=1;
    tmpzd->graph3d.draw=(int *) 
      malloc((mw->menu->mydata[i].md.choice.mygraph->anzN_RxRxR
	      +mw->menu->mydata[i].md.choice.mygraph->anzN_RxRxRp)*sizeof(int));
    for (k=0;(mw->menu->mydata[i].md.choice.mygraph->anzN_RxRxR
	      +mw->menu->mydata[i].md.choice.mygraph->anzN_RxRxRp)>k;k++)
      tmpzd->graph3d.draw[k]=0;

    tmpzd->graph3d.rough=1;
    tmpzd->graph3d.copy=1;
    tmpzd->graph3d.logx=0;
    tmpzd->graph3d.logy=0;
    tmpzd->graph3d.logz=0;
    sprintf(tmpzd->graph3d.Xlabel,"x");
    sprintf(tmpzd->graph3d.Ylabel,"y");
    sprintf(tmpzd->graph3d.Zlabel,"Z");
    tmpzd->graph3d.scaling=curve3dScaling;
    tmpzd->graph3d.xmin=curve3dxmin;
    tmpzd->graph3d.ymin=curve3dymin;
    tmpzd->graph3d.zmin=curve3dzmin;
    tmpzd->graph3d.xmax=curve3dxmax;
    tmpzd->graph3d.ymax=curve3dymax;
    tmpzd->graph3d.zmax=curve3dzmax;

    tmpzd->graph3d.persp_fakt=1;
    tmpzd->graph3d.scalefakt=1;
    tmpzd->graph3d.xfakt=1;
    tmpzd->graph3d.yfakt=1;
    tmpzd->graph3d.zfakt=1;
    tmpzd->graph3d.xangle=20;
    tmpzd->graph3d.yangle=20;
    tmpzd->graph3d.size=1;

    tmpmenu=mw->menu; /* needed in case of remapping of windows. 
			 See below. */
    StartMenu("3d Graph",0);
    StartMenu("Control",0);

      DefineBool("AutoScaling",&tmpzd->graph3d.scaling);
      DefineString("X label ",tmpzd->graph3d.Xlabel,500);
      DefineString("Y label ",tmpzd->graph3d.Ylabel,500);
      DefineString("Z label ",tmpzd->graph3d.Zlabel,500);
      DefineBool("logx",&tmpzd->graph3d.logx);
      DefineBool("logy",&tmpzd->graph3d.logy);
      DefineBool("logz",&tmpzd->graph3d.logz);
      DefineDouble("xmin",&tmpzd->graph3d.xmin);
      DefineDouble("xmax",&tmpzd->graph3d.xmax);
      DefineDouble("ymin",&tmpzd->graph3d.ymin);
      DefineDouble("ymax",&tmpzd->graph3d.ymax);
      DefineDouble("zmin",&tmpzd->graph3d.zmin);
      DefineDouble("zmax",&tmpzd->graph3d.zmax);
      StartMenu("3D options",0);
        DefineDouble("Persp fact",&tmpzd->graph3d.persp_fakt);
        DefineDouble("Scale fact",&tmpzd->graph3d.scalefakt);
        DefineDouble("x fact",&tmpzd->graph3d.xfakt);
        DefineDouble("y fact",&tmpzd->graph3d.yfakt);
        DefineDouble("z fact",&tmpzd->graph3d.zfakt);
        DefineDouble("xangle",&tmpzd->graph3d.xangle);
        DefineDouble("yangel",&tmpzd->graph3d.yangle);
        DefineDouble("size",&tmpzd->graph3d.size);
      EndMenu();
    EndMenu();
    for (k=0;k<mw->menu->mydata[i].md.choice.mygraph->anzN_RxRxR;k++){
      StartBoolMenu(mw->menu->mydata[i].md.choice.mygraph->N_RxRxRname[k],
		    &tmpzd->graph2d.draw[k]);
      ...
      */
      ...
      DefineMod("Color",
		&mw->menu->mydata[i].md.choice.mygraph->dN_RxRxR[k].color,
		NumberOfColors());
      DefineMod("Line Type",
		&mw->menu->mydata[i].md.choice.mygraph->dN_RxRxR[k].linetype,
		NumberOfLinetypes);
      DefineMod("Shape",
		&mw->menu->mydata[i].md.choice.mygraph->dN_RxRxR[k].shape,
		NumberOfShapes);
		
      DefineDouble("Size",&mw->menu->mydata[i].md.choice.mygraph->dN_RxRxR[k].size);
      DefineBool("Fill",&mw->menu->mydata[i].md.choice.mygraph->dN_RxRxR[k].fill);
      EndMenu();
    }
    for (j=0;j<mw->menu->mydata[i].md.choice.mygraph->anzN_RxRxRp;k++,j++){
      StartBoolMenu(mw->menu->mydata[i].md.choice.mygraph->N_RxRxRpname[j],
		    &tmpzd->graph2d.draw[k]);
      DefineMod("Color",
		&mw->menu->mydata[i].md.choice.mygraph->dN_RxRxRp[j].color,
		NumberOfColors());
      DefineMod("Line Type",
		&mw->menu->mydata[i].md.choice.mygraph->dN_RxRxRp[j].linetype,
		NumberOfLinetypes);
      DefineMod("Shape",
		&mw->menu->mydata[i].md.choice.mygraph->dN_RxRxRp[j].shape,
		NumberOfShapes);

      DefineDouble("Size",
		   &mw->menu->mydata[i].md.choice.mygraph->dN_RxRxRp[j].size);
      DefineBool("Fill",
		 &mw->menu->mydata[i].md.choice.mygraph->dN_RxRxRp[j].fill);
      EndMenu();
    }
    DefineItem(newdraw_);
    DefineItem(print_);
    ...
    mymenu->winp->type|=zeichentypemask;
    mymenu->winp->neu_zeichnenreq=0;
    mymenu->winp->neu_zeichnentype=curve_3d;
    mymenu->winp->neu_zeichnen=
      neu_zeichnen_3dgraph;
    mymenu->winp->installed=0;
    mymenu->winp->doublebuffer=1;
    mymenu->winp->newdraw=1;
    XQueryPointer(mw->myd,mw->menuw[i],&myroot,&mychild,&rx,&ry,&wx,&wy,&kb);
  ...
  case graph2d_:
    ...
    DefineDouble("Perspective factor",&tmpzd->nz1.persp_factor);
  ...
  case contour2d_:
    ...
    sprintf(tmpzd->nz2.Xlabel,"X");
    sprintf(tmpzd->nz2.Ylabel,"Y");
    tmpzd->nz2.hex=1;
    tmpzd->nz2.newdata=1;
    tmpzd->nz2.density=1;
    tmpzd->nz2.rough=1;
    ...
    tmpzd->nz2.draw_ufield= (int*) 
      malloc((mw->menu->mydata[i].md.choice.mygraph->anzNxN_RxR+mw->menu->mydata[i].md.choice.mygraph->anzNxN_RxRp)*sizeof(int));
    for (k=0;k<mw->menu->mydata[i].md.choice.mygraph->anzNxN_RxR+mw->menu->mydata[i].md.choice.mygraph->anzNxN_RxRp;k++)
    ...
    DefineString("X label ",tmpzd->nz2.Xlabel,500);
    DefineString("Y label ",tmpzd->nz2.Ylabel,500);
    ...
    DefineBool("Hex geometry",&tmpzd->nz2.hex);
    ...
    */
    ...
    for (k=0;k<mw->menu->mydata[i].md.choice.mygraph->anzNxN_RxR;k++){
      StartBoolMenu(mw->menu->mydata[i].md.choice.mygraph->NxN_RxRname[k],
		 &tmpzd->nz2.draw_ufield[k]);
      
      DefineMod("Color 1",
		&mw->menu->mydata[i].md.choice.mygraph->dNxN_RxR[k].veccol1,
		NumberOfColors());
      DefineMod("Color 2",
		&mw->menu->mydata[i].md.choice.mygraph->dNxN_RxR[k].veccol2,
		NumberOfColors());
      DefineDouble("Vec angle",
		   &mw->menu->mydata[i].md.choice.mygraph->dNxN_RxR[k].vecangle);
      DefineDouble("Vec Vecwidth",
		   &mw->menu->mydata[i].md.choice.mygraph->dNxN_RxR[k].vecwidth);
      DefineInt("Vec skip",
		&mw->menu->mydata[i].md.choice.mygraph->dNxN_RxR[k].vecstep);
      EndMenu();
    }
    for (int l=0;l<mw->menu->mydata[i].md.choice.mygraph->anzNxN_RxRp;l++,k++){
      StartBoolMenu(mw->menu->mydata[i].md.choice.mygraph->NxN_RxRpname[l],
		 &tmpzd->nz2.draw_ufield[k]);

      DefineMod("Color 1",
		&mw->menu->mydata[i].md.choice.mygraph->dNxN_RxRp[l].veccol1,
		NumberOfColors());
      DefineMod("Color 2",
		&mw->menu->mydata[i].md.choice.mygraph->dNxN_RxRp[l].veccol2,
		NumberOfColors());
      DefineDouble("Vec angle",
		   &mw->menu->mydata[i].md.choice.mygraph->dNxN_RxRp[l].vecangle);
      DefineDouble("Vec Vecwidth",
		   &mw->menu->mydata[i].md.choice.mygraph->dNxN_RxRp[l].vecwidth);
      DefineInt("Vec skip",
		&mw->menu->mydata[i].md.choice.mygraph->dNxN_RxRp[l].vecstep);
      EndMenu();
    }
    for (k=0;k<mw->menu->mydata[i].md.choice.mygraph->anzNxN_RxRxRt;k++){
  ...
  case contour3d_:
    ...
    DefineDouble("Perspective factor",&tmpzd->nzc.persp_factor);
  ...
  case freedraw_:
    tmpzd=(nzdata *) malloc(sizeof(nzdata));
    tmpzd->nzfree.draw=malloc(FreeDrawNumber()*sizeof(int));
    for (k=0;k<FreeDrawNumber();k++) 
      if (k==0) tmpzd->nzfree.draw[k]=1;
      else tmpzd->nzfree.draw[k]=0;
    tmpmenu=mw->menu; /* needed in case of remapping of windows. 
			 See below. */
    StartMenu("Graphics",0);
    for (k=0;k<FreeDrawNumber();k++)
      DefineBool(FreeDrawName(k), &tmpzd->nzfree.draw[k]);
    DefineItem(newdraw_);
    DefineItem(print_);
    DefineItem(close_);
    EndMenu();
    ...
    mymenu->winp->type|=zeichentypemask;
    mymenu->winp->neu_zeichnenreq=0;
    mymenu->winp->neu_zeichnentype=freedraw;
    mymenu->winp->neu_zeichnen=
      neu_zeichnen_freedraw;
    mymenu->winp->installed=0;
    mymenu->winp->doublebuffer=1;
    mymenu->winp->newdraw=1;
    XQueryPointer(mw->myd,mw->menuw[i],&myroot,&mychild,&rx,&ry,&wx,&wy,&kb);
  ...
  if (*wc>=wcmax){
    ...
    *mw=(mywindow *) realloc(*mw,wcmax*sizeof(mywindow));
    /* possible problem for reallocation: we have direct pointers
       ...
       */
    for (i=0;i<*wc;i++){ // what does this do???
