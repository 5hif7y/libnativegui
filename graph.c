/***************************************************************************\
graph library
Copyright (C) 1992-2017  Alexander Wagner
...
static int DefaultVecCol1=2, DefaultVecCol2=1,DefaultVecstep=1;
static double DefaultVecangle=3.14/4.,DefaultVecwidth=0.1;
...
void SetDefaultVecCol1(int c){DefaultVecCol1=c;}
void SetDefaultVecCol2(int c){DefaultVecCol2=c;}
void SetDefaultVecangle(double c){DefaultVecangle=c;}
void SetDefaultVecwidth(double c){DefaultVecwidth=c;}
void SetDefaultVecstep(int c){DefaultVecstep=c;}
...
void StartGraph(grstr *graph){
  ...
  graph->anzN_RxRxR    = 0;
  graph->maxanzN_RxRxR = 0;
  graph->N_RxRxRname   = NULL;
  graph->pN_RxRxR      = NULL;
  graph->dimN_RxRxR    = NULL;
  graph->dN_RxRxR      = NULL;
  graph->reqN_RxRxR    = NULL;
...
void InitGraph(){
  ...
  graph = (grstr *) calloc(maxgraph, sizeof(grstr));
  graphno=0;
  graphactive=0;
...
void NewGraph(){
  int old_max = maxgraph;
  graphno++;
  if (graphno>=maxgraph){
    maxgraph*=2;
    graph=(grstr *) realloc(graph,maxgraph*sizeof(grstr));
    memset(&(graph[old_max]), 0, (maxgraph - old_max) * sizeof(grstr));
  }
  StartGraph(&(graph[graphno]));
}
...
void DestroyGraph(grstr *graph){
  ...
  free(graph->N_RxRxRname);
  free(graph->reqN_RxRxR);
  free(graph->dimN_RxRxR);
...
void DefineGraphN_RxRxR(char *Name,double *data,int *dim,int *req){
  const static int optlen =10;

  if (graph==NULL) InitGraph();
  graph[graphno].anzN_RxRxR++;
  if (graph[graphno].anzN_RxRxR>=graph[graphno].maxanzN_RxRxR){
    graph[graphno].maxanzN_RxRxR+=optlen;
    graph[graphno].N_RxRxRname = (char **) 
      realloc(graph[graphno].N_RxRxRname,
	      graph[graphno].maxanzN_RxRxR*sizeof(char *));
    graph[graphno].dimN_RxRxR = (dim1 *) 
      realloc(graph[graphno].dimN_RxRxR,graph[graphno].maxanzN_RxRxR*sizeof(dim1));
    graph[graphno].pN_RxRxR = (double **) 
      realloc(graph[graphno].pN_RxRxR,
	      graph[graphno].maxanzN_RxRxR*sizeof(double *));
    graph[graphno].dN_RxRxR = (grdat *)
      realloc(graph[graphno].dN_RxRxR,graph[graphno].maxanzN_RxRxR*sizeof(grdat));
    graph[graphno].reqN_RxRxR = (int **)
      realloc(graph[graphno].reqN_RxRxR,graph[graphno].maxanzN_RxRxR*sizeof(int*));
  }
  graph[graphno].N_RxRxRname[graph[graphno].anzN_RxRxR-1]=
    (char *) malloc((strlen(Name)+1)*sizeof(char));
  sprintf(graph[graphno].N_RxRxRname[graph[graphno].anzN_RxRxR-1],"%s",Name);
  graph[graphno].dimN_RxRxR[graph[graphno].anzN_RxRxR-1][0]=dim;
  graph[graphno].pN_RxRxR[graph[graphno].anzN_RxRxR-1]=data;
  graph[graphno].reqN_RxRxR[graph[graphno].anzN_RxRxR-1]=req;
  graph[graphno].dN_RxRxR[graph[graphno].anzN_RxRxR-1].color=DefaultColor;
  graph[graphno].dN_RxRxR[graph[graphno].anzN_RxRxR-1].linetype=DefaultLineType;
  graph[graphno].dN_RxRxR[graph[graphno].anzN_RxRxR-1].shape=DefaultShape;
  graph[graphno].dN_RxRxR[graph[graphno].anzN_RxRxR-1].fill=DefaultFill;
  graph[graphno].dN_RxRxR[graph[graphno].anzN_RxRxR-1].size=DefaultSize;
}
...
void DefineGraphN_N(char *Name,int *data,int *dim,int *req){
  const static int optlen =10;

  if (graph==NULL) InitGraph();
  graph[graphno].anzN_N++;
  if (graph[graphno].anzN_N>=graph[graphno].maxanzN_N){
    graph[graphno].maxanzN_N+=optlen;
    graph[graphno].N_Nname = (char **) 
      realloc(graph[graphno].N_Nname,
	      graph[graphno].maxanzN_N*sizeof(char *));
    graph[graphno].dimN_N = (dim1 *) 
      realloc(graph[graphno].dimN_N,graph[graphno].maxanzN_N*sizeof(dim1));
    graph[graphno].pN_N = (double **) 
      realloc(graph[graphno].pN_N,
	      graph[graphno].maxanzN_N*sizeof(double *));
    graph[graphno].dN_N = (grdat *)
      realloc(graph[graphno].dN_N,graph[graphno].maxanzN_N*sizeof(grdat));
    graph[graphno].reqN_N = (int **)
      realloc(graph[graphno].reqN_N,graph[graphno].maxanzN_N*sizeof(int*));
  }
  graph[graphno].N_Nname[graph[graphno].anzN_N-1]=
    (char *) malloc((strlen(Name)+1)*sizeof(char));
  sprintf(graph[graphno].N_Nname[graph[graphno].anzN_N-1],"%s",Name);
  graph[graphno].dimN_N[graph[graphno].anzN_N-1][0]=dim;
  graph[graphno].pN_N[graph[graphno].anzN_N-1]=data;
  graph[graphno].reqN_N[graph[graphno].anzN_N-1]=req;
  graph[graphno].dN_N[graph[graphno].anzN_N-1].color=DefaultColor;
  graph[graphno].dN_N[graph[graphno].anzN_N-1].linetype=DefaultLineType;
  graph[graphno].dN_N[graph[graphno].anzN_N-1].shape=DefaultShape;
  graph[graphno].dN_N[graph[graphno].anzN_N-1].fill=DefaultFill;
  graph[graphno].dN_N[graph[graphno].anzN_N-1].size=DefaultSize;
}
...
void DefineGraphNxN_RxR(char *Name,double *data,
  ...
  if (graph[graphno].anzNxN_RxR>=graph[graphno].maxanzNxN_RxR){
    ...
    graph[graphno].dNxN_RxR = (grvecdat *)
      realloc(graph[graphno].dNxN_RxR,graph[graphno].maxanzNxN_RxR*sizeof(grvecdat));
  ...
  graph[graphno].dNxN_RxR[graph[graphno].anzNxN_RxR-1].veccol1=DefaultVecCol1;
  graph[graphno].dNxN_RxR[graph[graphno].anzNxN_RxR-1].veccol2=DefaultVecCol2;
  graph[graphno].dNxN_RxR[graph[graphno].anzNxN_RxR-1].vecangle=DefaultVecangle;
  graph[graphno].dNxN_RxR[graph[graphno].anzNxN_RxR-1].vecwidth=DefaultVecwidth;
  graph[graphno].dNxN_RxR[graph[graphno].anzNxN_RxR-1].vecstep=DefaultVecstep;
}

void DefineGraphNxN_RxRp(char *Name,double **data,
			int *mydim1,int *mydim2,int *req){
  const static int optlen =10;
  
  if (graph==NULL) InitGraph();
  graph[graphno].anzNxN_RxRp++;
  if (graph[graphno].anzNxN_RxRp>=graph[graphno].maxanzNxN_RxRp){
    graph[graphno].maxanzNxN_RxRp+=optlen;
    graph[graphno].NxN_RxRpname = (char **) 
      realloc(graph[graphno].NxN_RxRpname,graph[graphno].maxanzNxN_RxRp*sizeof(char *));
    graph[graphno].dNxN_RxRp = (grvecdat *)
      realloc(graph[graphno].dNxN_RxRp,graph[graphno].maxanzNxN_RxRp*sizeof(grvecdat));
    graph[graphno].dimNxN_RxRp = /*(dim2 *) What the hell is wrong here ? */ 
      realloc(graph[graphno].dimNxN_RxRp,graph[graphno].maxanzNxN_RxRp*sizeof(dim2));
    graph[graphno].pNxN_RxRp = (double ***) 
      realloc(graph[graphno].pNxN_RxRp,graph[graphno].maxanzNxN_RxRp*sizeof(double **));
    graph[graphno].reqNxN_RxRp = (int **)
      realloc(graph[graphno].reqNxN_RxRp,graph[graphno].maxanzNxN_RxRp*sizeof(int*));
  }
  graph[graphno].NxN_RxRpname[graph[graphno].anzNxN_RxRp-1]=Name;
  graph[graphno].dimNxN_RxRp[graph[graphno].anzNxN_RxRp-1][0]=mydim1;
  graph[graphno].dimNxN_RxRp[graph[graphno].anzNxN_RxRp-1][1]=mydim2;
  graph[graphno].pNxN_RxRp[graph[graphno].anzNxN_RxRp-1]=data;
  graph[graphno].reqNxN_RxRp[graph[graphno].anzNxN_RxRp-1]=req;
  graph[graphno].dNxN_RxRp[graph[graphno].anzNxN_RxRp-1].veccol1=DefaultVecCol1;
  graph[graphno].dNxN_RxRp[graph[graphno].anzNxN_RxRp-1].veccol2=DefaultVecCol2;
  graph[graphno].dNxN_RxRp[graph[graphno].anzNxN_RxRp-1].vecangle=DefaultVecangle;
  graph[graphno].dNxN_RxRp[graph[graphno].anzNxN_RxRp-1].vecwidth=DefaultVecwidth;
  graph[graphno].dNxN_RxRp[graph[graphno].anzNxN_RxRp-1].vecstep=DefaultVecstep;
}
...
void DefineGraphNxN_N(char *Name,int *data,
                      int *mydim1,int *mydim2,int *req){
  const static int optlen =10;
  
  if (graph==NULL) InitGraph();
  graph[graphno].anzNxN_N++;
  if (graph[graphno].anzNxN_N>=graph[graphno].maxanzNxN_N){
    graph[graphno].maxanzNxN_N+=optlen;
    graph[graphno].NxN_Nname = (char **) 
      realloc(graph[graphno].NxN_Nname,graph[graphno].maxanzNxN_N*sizeof(char *));
    graph[graphno].dimNxN_N = (dim2 *) 
      realloc(graph[graphno].dimNxN_N,graph[graphno].maxanzNxN_N*sizeof(dim2));
    graph[graphno].pNxN_N = (double **) 
      realloc(graph[graphno].pNxN_N,graph[graphno].maxanzNxN_N*sizeof(double *));
    graph[graphno].reqNxN_N = (int **)
      realloc(graph[graphno].reqNxN_N,graph[graphno].maxanzNxN_N*sizeof(int*));
  }
  graph[graphno].NxN_Nname[graph[graphno].anzNxN_N-1]=
    (char *) malloc((strlen(Name)+1)*sizeof(char));
  sprintf(graph[graphno].NxN_Nname[graph[graphno].anzNxN_N-1],"%s",Name);
  graph[graphno].dimNxN_N[graph[graphno].anzNxN_N-1][0]=mydim1;
  graph[graphno].dimNxN_N[graph[graphno].anzNxN_N-1][1]=mydim2;
  graph[graphno].pNxN_N[graph[graphno].anzNxN_N-1]=data;
  graph[graphno].reqNxN_N[graph[graphno].anzNxN_N-1]=req;
}
  ...
  */
  ...
  case N_RxR:
    ...
    *gp=graph.pN_RxR[get];
    break;
  case N_RxRxR:
    rx_size=/*copy[0]**/*graph.dimN_RxRxR[get][0];
    no[0]=rx_size;
    *gp=graph.pN_RxRxR[get];
    break;
  case N_RxRxRxR:
  ...
  case NxN_RxR:
    ...
    *gp=graph.pNxN_RxR[get];
    break;
  case NxN_RxRp:
    rx_size=/*copy[0]**/*graph.dimNxN_RxRp[get][0];
    ry_size=/*copy[1]**/*graph.dimNxN_RxRp[get][1];
    no[0]=rx_size;
    no[1]=ry_size;
    *gp=*graph.pNxN_RxRp[get];
    break;
  case NxN_RxRxRt:
